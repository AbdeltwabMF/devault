...
contract Storage {
    ...
    mapping(string => uint256) private _hashToIndex;
    ...
    function storeFile(
        ...
        require(
            bytes(_mimeType).length <= 128,
            "Mime type cannot be longer than 128 bytes"
        );
     ...
     */
    function getFilesLength() public view returns (uint256) {
     ...
     */
    function shareFile(address _to, uint256 _index) public returns (bool) {
        require(
            _index < _ownerToFiles[msg.sender].length,
            "Index out of bounds!"
        );
     ...
     * @param _index Index of the file
     */
    function removeFile(uint256 _index) public returns (bool) {
        require(
            _index < _ownerToFiles[msg.sender].length,
            "Index out of bounds!"
        );
